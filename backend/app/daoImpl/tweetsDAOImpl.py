...
class tweetsDAOImpl(TweetsDAO):
    # Guarda un tweet en el perfil del usuario
    def save_tweet_user(db, user_id: int, tweet: schemas.TweetSavedCreate):
    ...
    def delete_tweet_user(db: Session, tweet: schemas.TweetSaved):
        db.delete(tweet)
        db.commit()
