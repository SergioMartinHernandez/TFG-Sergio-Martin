...
from dao import tweetsDAO, searchsDAO
...
@router.post("/user/savetweet/{tweet_id}", response_model=schemas.TweetSaved)
def save_tweet_user(tweet_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_user)):
    tweet = searchsDAO.get_tweet_by_id(db=db, tweetsearch_id=tweet_id)
    tweets_saved = tweetsDAO.save_tweet_user(db=db, user_id=current_user.id, tweet=tweet)
    return tweets_saved
