...
def create_tweet_search_for_search(
    search_id: int, tweetsearch: schemas.TweetSearchCreate, db: Session = Depends(get_db)
):
    return crud.create_user_search(db=db, tweetsearch=tweetsearch, search_id=search_id)


@app.get("/tweetsearchs/", response_model=list[schemas.TweetSearch])
...
def create_user_search_for_search(
    search_id: int, usersearch: schemas.UserSearchCreate, db: Session = Depends(get_db)
):
    return crud.create_user_search(db=db, usersearch=usersearch, search_id=search_id)
